        ...
        >
          ...
          <div className="flex items-start gap-3 min-w-0 mb-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className={`text-sm text-slate-500 leading-relaxed mt-1 ${isArabic(r.text) ? 'text-right' : 'text-left'}`}>{r.text}</p>
            </div>
          </div>

          {/* Actions en bas, flex-wrap pour mobile */}
          <div className="flex items-center gap-2 flex-wrap pt-2 border-t border-slate-100">
            ...
            {/* Corbeille poussée à droite */}
            <button
              onClick={() => onDelete(r.id)}
              className="ml-auto p-1.5 text-slate-400 hover:text-rose-400 hover:bg-rose-50 rounded-lg transition-all"
            >
              <Trash2 size={16} />
            </button>
          </div>
